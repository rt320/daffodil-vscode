...
function checkItemsOnLine(
  ...
  if (itemsOnLine == 1 || itemsOnLine == 0) {
    if (
      ...
      if (itemsOnLine == 0) {
        // going back 3 chars inserted this between ref= and ""
  ...
  if (itemsOnLine > 1) {
    const tagPos = triggerText.lastIndexOf('<' + nsPrefix + nearestTagNotClosed)
    const nextTagPos = triggerText.indexOf('<', tagPos + 1)

    if (
      triggerText.substring(tagPos, nextTagPos).includes('>') ||
